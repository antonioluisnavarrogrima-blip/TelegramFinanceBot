...
# 2. Add Env Vars for FMP and Webhook
content = content.replace(
    'GEMINI_API_KEY   = os.getenv("GEMINI_API_KEY")',
    'GEMINI_API_KEY   = os.getenv("GEMINI_API_KEY")\nFMP_API_KEY      = os.getenv("FMP_API_KEY")\nRENDER_EXTERNAL_URL = os.getenv("RENDER_EXTERNAL_URL", "")'
)

# 3. Clean global caches, thread lock, locks, tokenbucket
content = re.sub(r'class TokenBucket:.*?def _es_consulta_financiera', 'def _es_consulta_financiera', content, flags=re.DOTALL)

# Find and replace YF Caches globals
content = re.sub(r'# --- CACHÉ YFINANCE Y POOLING \(THREAD-SAFE\) ---.*?def _obtener_info_yf', 'def _obtener_info_fmp', content, flags=re.DOTALL)

# Delete fabricante_de_graficos, es_url_valida etc? Actually, let's just rewrite _obtener_info_yf and the fundamental checkers completely.
# I'll do this by replacing from "def _obtener_info_yf" to "def _get_pipeline_semaphore():" wholesale.

# But wait, there's _get_yf_session() before _obtener_info_yf!
content = re.sub(r'def _get_yf_session.*?def _obtener_info_fmp', 'def _obtener_info_fmp', content, flags=re.DOTALL)

# 4. Replace METRICAS_YF with METRICAS_FMP
...
# We can provide the new checkers and fmp function as a string to inject.
new_checkers = """
async def _obtener_info_fmp(ticker: str, clase: str = "ACCION") -> dict:
    cached = await db.obtener_yf_cache(ticker)
    ...
    if data:
        await db.guardar_yf_cache(ticker, data, clase)
...
async def _chequear_fundamentales_accion(ticker: str, filtros: dict) -> dict | None:
    try:
        ...
        if div_yield_dec is None: div_yield_dec = 0
        div_yield = div_yield_dec
        div_rate = info.get('lastDiv', 0)
        ...
        return {
            ...
            "div_rate_abs": round(div_rate, 2) if div_rate else 0.0,
        }
    ...
    try:
        ...
        for f in filtros_extra:
            if f["metrica"] == "dividend_yield":
                ...
                if not OPS.get(f["operador"], operator.lt)(p_ffo_proxy, f["valor"]): return None
...
async def _chequear_fundamentales_etf(ticker: str, filtros_extra: list) -> dict | None:
    try:
        ...
        if not info: return None
        ter = 0  # FMP profile no tiene TER de forma nativa en endpoint gratuito. Se asume 0 para no bloquear.
        # Fallback a AUM si hay mktCap
        aum = info.get('mktCap', 0) or 0
        div_yield = info.get('dividendYieldTTM', 0) or 0
        for f in filtros_extra:
            if f["metrica"] == "ter":
                # Lo pasamos transparente
                pass
            elif f["metrica"] == "aum":
...
def fabricante_de_graficos(ticker: str, periodo: str = "3mo") -> tuple[bytes | None, float | None]:
    return None, 0.0
"""
content = re.sub(r'def _obtener_info_fmp.*?def fabricante_de_graficos.*?return None, None', new_checkers, content, flags=re.DOTALL)

# Delete async def _chequeo_asincrono entirely
...
# Update gather loops in _pipeline_hibrido_interno
content = content.replace(
...
# Update "fabricante de graficos" usage. 
# In _pipeline_hibrido_interno:
...
# Update lifespan to use webhook logic and remove polling
...
async def lifespan(app: FastAPI):
    ...
    await db.precargar_semillas_basicas()    # Asegura operatividad inmediata
    ...
    logger.info("[LIFESPAN] Bot de Telegram configurado en modo Webhook.")
...
async def webhook_telegram(request: Request):
    try:
        ...
        # Delegate process to telegram app immediately
...
content = content.replace('@web_app.post("/webhook-pago")', webhook_route + '@web_app.post("/webhook-pago")')

with open('bot_patched.py', 'w', encoding='utf-8') as f:
    f.write(content)
